...
// Server2 implements EmbedService RPC handlers using Rust FFI backend.
...
// NewServer2 creates a new RPC server instance using Rust FFI backend.
// Does not initialize the model - client must call Initialize() RPC first.
func NewServer2(ctx context.Context, libDir, modelDir string, dimensions int, idleTimeout time.Duration) (*Server2, error) {
	s := &Server2{
		model:       nil,
...
// Runs in background goroutine started by NewServer2.
func (s *Server2) monitorIdle() {
