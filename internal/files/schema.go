...
func NewSchemaRegistry() *SchemaRegistry {
	return &SchemaRegistry{
		tables: map[string]TableSchema{
			...
			"modules": NewTableSchema("modules",
				"module_path",
				"file_count",
				"line_count_total",
				"line_count_code",
...
func (sr *SchemaRegistry) ValidateTableAndColumn(table, column string) error {
	...
	if !ok {
		return &ValidationError{
			...
			Hint:    "Valid tables: files, types, type_fields, functions, function_parameters, type_relationships, function_calls, imports, chunks, modules, cache_metadata",
		}
	}
