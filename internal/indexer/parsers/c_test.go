...
func TestCParser_ParseStruct(t *testing.T) {
	...
	extraction, err := parser.ParseFile(ctx, "../../../testdata/code/c/simple.c")

	require.NoError(t, err)
	require.NotNil(t, extraction)

	// Verify language detection
	assert.Equal(t, "c", extraction.Language)
	assert.Contains(t, extraction.FilePath, "simple.c")
...
func TestCParser_ParseFunctions(t *testing.T) {
	...
	require.NotNil(t, extraction.Symbols)
	require.GreaterOrEqual(t, len(extraction.Symbols.Functions), 4) // create_repository, add_user, find_user, free_repository

	functionNames := make(map[string]SymbolInfo)
...
func TestCParser_ParseGlobalVariables(t *testing.T) {
	...
	require.NotNil(t, extraction.Data)
	require.GreaterOrEqual(t, len(extraction.Data.Variables), 1, "Should extract at least one global variable") // connection_count
...
func TestCParser_CountIncludes(t *testing.T) {
	...
	require.NotNil(t, extraction.Symbols)
	assert.Equal(t, 3, extraction.Symbols.ImportsCount) // stdio.h, stdlib.h, string.h
}
...
func TestCParser_LineNumbers(t *testing.T) {
	...
	assert.Greater(t, extraction.EndLine, 40) // File has ~49 lines
...
func TestCParser_InvalidFile(t *testing.T) {
	...
	extraction, err := parser.ParseFile(ctx, "../../../testdata/code/c/nonexistent.c")
...
func TestCPPParser_ParseClass(t *testing.T) {
	...
	extraction, err := parser.ParseFile(ctx, "../../../testdata/code/cpp/simple.cpp")

	require.NoError(t, err)
	require.NotNil(t, extraction)

	// Verify language detection
	assert.Equal(t, "cpp", extraction.Language)
	assert.Contains(t, extraction.FilePath, "simple.cpp")

	// Verify class extraction - looking for User, Point, or Repository
	require.NotNil(t, extraction.Symbols)
	require.GreaterOrEqual(t, len(extraction.Symbols.Types), 1, "Should extract at least one type")
	...
	require.NotNil(t, extraction.Definitions)
	assert.GreaterOrEqual(t, len(extraction.Definitions.Definitions), 1, "Should have at least one definition")
}
...
func TestCPPParser_ParseMethods(t *testing.T) {
	...
	if len(extraction.Symbols.Functions) > 0 {
		for _, fn := range extraction.Symbols.Functions {
			assert.Greater(t, fn.StartLine, 0, "Function %s should have valid start line", fn.Name)
			assert.GreaterOrEqual(t, fn.EndLine, fn.StartLine, "Function %s end line should be >= start line", fn.Name)
...
func TestCPPParser_ParseGlobalVariables(t *testing.T) {
	...
	if len(extraction.Data.Variables) > 0 {
		for _, variable := range extraction.Data.Variables {
			assert.Greater(t, variable.StartLine, 0, "Variable %s should have valid start line", variable.Name)
...
func TestCPPParser_CountIncludes(t *testing.T) {
	...
	// Verify includes count
	require.NotNil(t, extraction.Symbols)
	assert.Equal(t, 3, extraction.Symbols.ImportsCount) // string, vector, memory
}
...
func TestCPPParser_LineNumbers(t *testing.T) {
	...
	// Verify file range
	assert.Equal(t, 1, extraction.StartLine)
	assert.Greater(t, extraction.EndLine, 60) // File has ~65 lines
...
func TestCPPParser_InvalidFile(t *testing.T) {
	...
	// Test: Non-existent file returns error
	extraction, err := parser.ParseFile(ctx, "../../../testdata/code/cpp/nonexistent.cpp")
...
func TestCParser_EmptyExtractionStructure(t *testing.T) {
	...
	assert.NotNil(t, extraction.Symbols, "Symbols tier should be initialized")
	assert.NotNil(t, extraction.Definitions, "Definitions tier should be initialized")
	assert.NotNil(t, extraction.Data, "Data tier should be initialized")
